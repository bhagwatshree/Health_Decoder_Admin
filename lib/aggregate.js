...
export async function recentRequests(limit = 25) {
  const { rows } = await db.query(
    `SELECT e.id, e.created_at, e.provider, e.operation, e.model, e.latency_ms, e.cost_usd, e.success,
            e.gemini_key_index, e.input_tokens, e.output_tokens, e.units,
...
export async function tokenUsageByCustomer(days, limit = 20) {
  ...
  function blank(extra) {
    return { requests: 0, inputTokens: 0, outputTokens: 0, units: 0, totalCost: 0, ...extra };
  }
  function accumulate(target, r, inTok, outTok, units) {
  ...
  function derive(o) {
    ...
    o.ioRatio = o.outputTokens > 0 ? o.inputTokens / o.outputTokens : null;
  ...
  for (const r of rows) {
    ...
    accumulate(c, r, inTok, outTok, units);

    if (!c.byProvider.has(r.provider)) c.byProvider.set(r.provider, blank({ provider: r.provider }));
    accumulate(c.byProvider.get(r.provider), r, inTok, outTok, units);

    c.byModel.push(derive({
      provider: r.provider, model: r.model, requests: r.requests,
      inputTokens: inTok, outputTokens: outTok, units, totalCost: r.total_cost || 0,
    }));

    if (!providerTotals.has(r.provider)) providerTotals.set(r.provider, blank({ provider: r.provider }));
    accumulate(providerTotals.get(r.provider), r, inTok, outTok, units);
    accumulate(overall, r, inTok, outTok, units);
  }
