...
/** Fetch current status of CloudWatch Alarms for the stack or MedicalScanner namespace. */
export async function getCloudWatchAlarmStatuses() {
  try {
    ...
    if (!MetricAlarms || MetricAlarms.length === 0) {
      // Fallback attempt to search for Fraud alarms if stack name prefix yields none
      const fraudRes = await cw.send(
        new DescribeAlarmsCommand({
          AlarmNamePrefix: 'medical-scanner',
        })
      );
      return (fraudRes.MetricAlarms || []).map(formatAlarm);
    }
    return MetricAlarms.map(formatAlarm);
  } catch (err) {
    console.error('DescribeAlarmsCommand failed:', err.message);
    return [];
  }
}
...
function formatAlarm(a) {
  return {
    ...
    stateValue: a.StateValue, // 'OK' | 'ALARM' | 'INSUFFICIENT_DATA'
